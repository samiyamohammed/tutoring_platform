...
class EnrollmentService {
  async enrollStudent(data) {
    ...
    const existingEnrollment = await EnrollmentRepository.findByStudentId(
      student
    );
    if (
      existingEnrollment.some(
  ...
  async getCurrentEnrollment(studentId, courseId) {
    const enrollment = await EnrollmentRepository.findByStudentAndCourseId(
      studentId,
      courseId
    );
    if (!enrollment) throw new Error("Enrollment not found");
  ...
  async updateProgress(
    enrollmentId,
    studentId,
    { moduleId, sectionId, timeSpent = 1 }
  ) {
    const enrollment = await this._verifyEnrollmentOwnership(
      enrollmentId,
      studentId
    );

    const moduleProgress = this._getOrCreateModuleProgress(
      enrollment,
      moduleId
    );
    const sectionProgress = this._getOrCreateSectionProgress(
      moduleProgress,
      sectionId
    );
  ...
  async markSectionComplete(enrollmentId, studentId, { moduleId, sectionId }) {
    ...
    const allSectionsCompleted = moduleProgress.sections.every(
  ...
  async _verifyEnrollmentOwnership(enrollmentId, studentId) {
    ...
    if (!enrollment) throw new Error("Enrollment not found");
    if (!enrollment.student.equals(studentId))
  ...
  _getOrCreateSectionProgress(moduleProgress, sectionId) {
    let sectionProgress = moduleProgress.sections.find((s) =>
  ...
  _getModuleProgress(enrollment, moduleId) {
    ...
    if (!module)
      throw new Error(`Module progress not found for moduleId: ${moduleId}`);
    return module;
  }

  _getSectionProgress(moduleProgress, sectionId) {
    const section = moduleProgress.sections.find((s) =>
      s.sectionId.equals(sectionId)
    );
    if (!section)
      throw new Error(`Section progress not found for sectionId: ${sectionId}`);
    return section;
  }
}
