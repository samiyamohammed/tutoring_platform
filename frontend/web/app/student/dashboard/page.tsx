    ...
    const fetchStudentData = async () => {
      try {
        ...
        const enrollmentsRes = await fetch(
          "http://localhost:5000/api/enrollment/mycourses",
          {
            method: "GET",
            headers: {
        ...
        const hoursStudied = Math.round(
          fetchedEnrollments.reduce(
            (acc: number, e: Enrollment) =>
              acc + (e.progress?.timeSpentTotal || 0),
            0
          ) / 3600
        );
        ...
        const completedModules = fetchedEnrollments.reduce(
          (acc: number, e: Enrollment) =>
            acc +
            (e.progress?.modules?.filter(
              (m: { status: string }) => m.status === "completed"
            ).length || 0),
          0
        );

        const completedSections = fetchedEnrollments.reduce(
          (acc: number, e: Enrollment) =>
            acc +
            (e.progress?.modules?.reduce(
              (
                modAcc: number,
                mod: Enrollment["progress"]["modules"][number]
              ) =>
                modAcc +
                (mod.sections?.filter(
                  (
                    s: Enrollment["progress"]["modules"][number]["sections"][number]
                  ) => s.status === "completed"
                ).length || 0),
              0
            ) || 0),
          ...
          <div className="flex-1 space-y-4 p-8 pt-6">
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {stats.completedModules} modules
                  </div>
                  <p className="text-xs text-muted-foreground">
                    ...
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                      {enrollments.map((enrollment) => {
                        ...
                        const completedModules =
                          enrollment.progress?.modules?.filter(
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        Your learning activity across courses
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="space-y-4">
                        {enrollments.map((enrollment) => {
                          ...
                          const hours = Math.round(
                            enrollment.progress.timeSpentTotal / 3600
                          );
                          return (
                  ...
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        Your progress towards completing courses
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="space-y-4">
                    ...
                    {enrollments.length === 0 ? (
                      <div className="flex flex-col items-center justify-center py-12">
                        ...
                        <p className="text-muted-foreground">
                          Enroll in courses to see scheduled sessions
                        </p>
                      </div>
                    ) : viewMode === "calendar" ? (
                      <CalendarView
                      ...
                      <div className="space-y-4">
                        {getUpcomingSessions(enrollments).length === 0 ? (
                          ...
                          getUpcomingSessions(enrollments).map((session) => (
                            <SessionCard
                              key={`${session.courseId}-${session.day}-${session.startTime}`}
                              session={session}
                            />
                          ))
                        )}
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      {(() => {
                        ...
                        const enrolledCategories = enrollments.map(
                          (e) => e.course.category
                        );
                        ...
                        return recommendedCourses.length === 0 ? (
                          <div className="flex flex-col items-center justify-center py-12">
                            ...
                            <p className="text-muted-foreground">
                              Enroll in more courses to get personalized
                              recommendations
                            </p>
                          </div>
                        ) : (
                          recommendedCourses.map((course, idx) => (
                            <div key={idx} className="flex items-center">
                              ...
                              <div className="flex-1 space-y-1">
                                ...
                                <p className="text-xs text-muted-foreground">
                                  {course.tutor.name} • {course.rating || "N/A"}{" "}
