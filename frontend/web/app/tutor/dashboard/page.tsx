...
export default function TutorDashboardPage() {
  ...
  const [sessionTypeDistribution, setSessionTypeDistribution] = useState<ChartData[]>([])
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const sessionsRes = await fetch(`http://localhost:5000/api/session?courses=${courseIds.join(',')}`, { headers })
        if (!sessionsRes.ok) throw new Error('Failed to fetch sessions')
        const sessionsData = await sessionsRes.json()
        setSessions(sessionsData)

        // Calculate stats
        const activeStudents = tutorEnrollments.filter(
        ...
        const pendingEarnings = tutorEnrollments.reduce(
          (sum: number, e: Enrollment) => sum +
            (e.payment?.status === 'pending' || e.payment?.status === 'partial' ?
              (e.payment?.totalAmount || 0) - (e.payment?.amountPaid || 0) : 0), 0
        )

        // Calculate session type distribution
        const sessionTypeCounts = tutorEnrollments.reduce((acc: Record<string, number>, e: Enrollment) => {
          acc[e.enrolledSessionType] = (acc[e.enrolledSessionType] || 0) + 1
          return acc
        }, {})

        const sessionDistribution = Object.entries(sessionTypeCounts).map(([name, value]) => ({
        ...
        if (timeRange === 'week') {
          // Last 7 days
          enrollmentTimeData = Array(7).fill(0).map((_, i) => {
          ...
          earningsTimeData = Array(7).fill(0).map((_, i) => {
            const date = new Date(now)
            date.setDate(date.getDate() - 6 + i)
            const dateStr = date.toLocaleDateString('en-US', { weekday: 'short' })
            ...
            const amount = tutorEnrollments
              ...
              .reduce((sum: number, e: Enrollment) => sum + (e.payment?.amountPaid || 0), 0)
          ...
          enrollmentTimeData = Array(weeks).fill(0).map((_, i) => {
            const startDay = i * 7
          ...
          earningsTimeData = Array(weeks).fill(0).map((_, i) => {
            const startDay = i * 7
            const endDay = Math.min(startDay + 6, 29)
          ...
          // Last 12 months
          enrollmentTimeData = Array(12).fill(0).map((_, i) => {
            const date = new Date(now)
          ...
          earningsTimeData = Array(12).fill(0).map((_, i) => {
            const date = new Date(now)
            date.setMonth(date.getMonth() - 11 + i)
            const monthStr = date.toLocaleDateString('en-US', { month: 'short' })
        ...
        const coursePerformance = coursesData.map((course: Course) => {
          ...
          const avgCompletion = courseEnrollments.length > 0
            ? courseEnrollments.reduce(
              (sum: number, e: Enrollment) => sum + (e.progress?.completionPercentage || 0), 0
            ) / courseEnrollments.length
            : 0

          const avgTimeSpent = courseEnrollments.length > 0
            ? courseEnrollments.reduce(
              (sum: number, e: Enrollment) => sum + (e.progress?.timeSpentTotal || 0), 0
            ) / courseEnrollments.length
            : 0
        ...
        const mockStats: DashboardStats = {
          ...
          pendingEarnings,
          changeFromLastMonth: {
            students: Math.round((activeStudents / Math.max(1, activeStudents - 14)) * 100 - 100 || 0),
            courses: Math.round((activeCourses / Math.max(1, activeCourses - 2)) * 100 - 100 || 0),
            earnings: Math.round((totalEarnings / Math.max(1, totalEarnings * 0.82)) * 100 - 100 || 0)
          }
        }
        setStats(mockStats)
        ...
        <main className="flex flex-col">
          <div className="flex items-center justify-between border-b px-4 py-3">
            ...
            <div className="flex items-center gap-2">
              <Select value={timeRange} onValueChange={(value: "week" | "month" | "year") => setTimeRange(value)} disabled={loading}>
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
                  ...
                  <Card className="col-span-3">
                    <CardHeader>
                      ...
                      <CardDescription>How students are taking your courses</CardDescription>
                    </CardHeader>
                    <CardContent className="pl-2">
                      {loading ? (
                        <div className="h-[300px] w-full flex items-center justify-center">
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
                  ...
                  <Card className="col-span-4">
                    <CardHeader>
                      ...
                      <CardDescription>Average completion across your courses</CardDescription>
                    </CardHeader>
                    <CardContent className="pl-2">
                      {loading ? (
                        ...
                        <div className="h-[250px] w-full">
                          <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={performanceData}>
                              ...
                              <Tooltip
                                contentStyle={{ backgroundColor: 'hsl(var(--background))', borderRadius: 'var(--radius)' }}
                                formatter={(value: number) => [`${value}%`, 'Average Completion']}
                              />
              ...
              <TabsContent value="analytics" className="space-y-4">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>Track how students are progressing through your courses</CardDescription>
                  </CardHeader>
                  <CardContent className="pl-2">
                    {loading ? (
                              ...
                              <div className="flex-1 space-y-1">
                                <div className="flex items-center justify-between">
                                  <p className="text-sm font-medium leading-none">
                                    {enrollment.student?.name || enrollment.student?.email || `Student ${enrollment._id.slice(-4)}`}
                                  </p>
