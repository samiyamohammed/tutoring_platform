...
export default function DocumentVerificationPage() {
  ...
  useEffect(() => {
    const fetchTutorData = async () => {
      try {
        ...
        if (isReapply && data.verification_status === "rejected") {
          await fetch(`http://localhost:5000/api/users/${data._id}`, {
            method: "PUT",
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        form.reset({
          ...
          verification_status: isReapply ? "initial" : data.verification_status || "initial",
  ...
  async function onSubmit(values: DocumentVerificationFormValues) {
    ...
    try {
      ...
      formData.append("verification_rejection_reason", "");
      ...
      const fileDocumentNames: string[] = [];

      // 3. Append files with proper structure
      values.documents.forEach((doc) => {
        doc.files.forEach((fileObj) => {
          if (fileObj.file instanceof File) {
            formData.append("files", fileObj.file); // Must match Multer's field name
            fileDocumentNames.push(fileObj.name || fileObj.file.name);
          }
        });
      });

      // 4. Append document names array
      formData.append("fileDocumentNames", JSON.stringify(fileDocumentNames));
